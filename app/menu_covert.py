...
class ESP32MenuConverter:
    ...
    def check_for_updates(self):
        ...
        try:
            # gitee项目信息
            gitee_repo_url = "https://gitee.com/lzplds/esp32-menu-zh.git"
            api_url = "https://gitee.com/api/v5/repos/lzplds/esp32-menu-zh/releases/latest"
            ...
            if self.compare_versions(self.version, latest_version) < 0:
                ...
                # 询问用户是否更新
                update_confirm = input(f"\n{Colors.MAGENTA}是否更新到最新版本？(y/n): {Colors.END}").strip().lower()
                if update_confirm == 'y':
            ...
            print(f"{Colors.YELLOW}请手动访问项目页面检查更新: {gitee_repo_url}{Colors.END}")
    ...
    def perform_update(self, download_url):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # 下载更新包
                print(f"{Colors.WHITE}下载更新包...{Colors.END}")
                ...
                print(f"{Colors.GREEN}请重新启动工具以应用更新{Colors.END}")
                
        except Exception as e:
            print(f"{Colors.RED}\n更新失败: {str(e)}{Colors.END}")
            print(f"{Colors.YELLOW}建议手动从项目页面下载最新版本并替换文件{Colors.END}")
            print(f"{Colors.YELLOW}项目地址: https://gitee.com/lzplds/esp32-menu-zh.git{Colors.END}")
